def adams_bashforth_integration(f, x0, y0, h, n):
    """
    Función para realizar la integración numérica utilizando el método de Adams-Bashforth de paso múltiple.
    ...
    for i in range(n):
        ...
        if i < 2:
            ...
            # Utilizar el método de Adams-Bashforth de paso múltiple
            y_new = y + (h / 12) * (
                23 * f(x, y) - 16 * f(x_values[-2], y_values[-2]) + 5 * f(x_values[-3], y_values[-3])
            )
...
x_values, y_values = adams_bashforth_integration(equation, x0, y0, h, n)
